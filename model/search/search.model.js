...
exports.receive_challenge = async function (req, res){ 
    try{
        ...
        if(data == ""){//check already exist
            ...
            const [results, teams_challenges] = await sequelize.query("SELECT p.teamname,p.gender,p.age_range,p.type,p.token_id as opponent_token_id,p.`profile_img`,p.matches,p.won,p.draw,pc.id AS challenge_id,s.name AS sport_name,pc.match_contest,pc.message AS challenge_message,pc.added_date AS date FROM `teams_challenges` pc INNER JOIN `teams` p ON pc.`team_id` = p.`id` INNER JOIN `sports` s ON s.`id` = pc.`sport_id` WHERE pc.accept_status = 0 AND pc.opponent_id = ? AND p.`status` = 1 AND s.`status` = 1 AND pc.block = 0 ORDER BY pc.id DESC LIMIT "+limit+" OFFSET "+offset+"",{replacements:[team_id]});
            if(teams_challenges!=""){
    ...
    try{
        ...
        if(data == ""){//check already exist
            ...
            if(teams_challenges!=""){
                ...
                await sequelize.query("INSERT INTO notifications (team_id,opponent_id,description,link,date) VALUES (?,?,?,?,?)",{replacements:[opponent_team_id,team_id,description,link,currenttime]});
                /* End add notification */
