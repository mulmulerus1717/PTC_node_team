...
exports.receive_challenge = async function (req, res){ 
    try{
        ...
        if(data == ""){//check already exist
            ...
            const [results, teams_challenges] = await sequelize.query("SELECT pc.id AS team_player_id,pc.message,p.firstname,p.lastname,p.gender,p.age,p.token_id,p.`profile_img`,p.matches,p.won,p.draw,p.teams,p.teams_matches,p.teams_won,p.teams_draw,s.name AS sport_name,(SELECT GROUP_CONCAT(IF(pkc.won=p.id && pkc.draw=0,'W',IF(pkc.draw=1,'D','L'))) FROM players_challenges pkc WHERE (pkc.player_id = p.id OR pkc.opponent_id = p.id) and pkc.accept_status = 1 AND (pkc.won != 0 or pkc.draw != 0) ORDER BY pkc.id DESC LIMIT 5) as last_matches FROM `team_players` pc INNER JOIN `players` p ON pc.`player_id` = p.`id` INNER JOIN `sports` s ON s.`id` = pc.`sport_id` WHERE pc.status = 0 AND pc.team_id = ? AND p.`status` = 1 AND s.`status` = 1 ORDER BY pc.id DESC LIMIT "+limit+" OFFSET "+offset+"",{replacements:[team_id]});
            if(teams_challenges!=""){
    ...
    try{
        ...
        if(data == ""){//check already exist
            ...
            if(teams_challenges!=""){
                ...
                var total_teams_count = 0;
                const [results, teams_counts] = await sequelize.query("SELECT count(1) AS total FROM `team_players` pc WHERE pc.status = 1 AND pc.player_id = ?",{replacements:[player_id]});
                if(teams_counts!=""){
                    total_teams_count = teams_counts[0].total;
                }
                await sequelize.query("UPDATE players SET teams = ? WHERE id = ?",{replacements:[total_teams_count,player_id]});
...
exports.reject_request = async function (req, res){ 
    try{
        ...
        if(data == ""){//check already exist
            ...
            var team_id = data[0].id;
            const [results, teams_challenges] = await sequelize.query("SELECT player_id FROM `team_players` pc WHERE pc.team_id = ? AND pc.id = ?",{replacements:[team_id,team_player_id]});
            if(teams_challenges!=""){
                var player_id = teams_challenges[0].player_id;
...
/* Team players */
exports.team_players = async function (req, res){ 
    try{
        res.setHeader('Content-Type','application/json');
        var authorization = req.header('authorization');
        var auth_token = (authorization).split(" ")[1];
        ...
        var offset = req.body.offset != undefined ? req.body.offset : 0;
        ...
        if(data == ""){//check already exist
            ...
            var team_id = data[0].id;
            const [resultsChallenges, challenges_total] = await sequelize.query("SELECT 1 FROM `team_players` pc INNER JOIN `players` p ON pc.`player_id` = p.`id` INNER JOIN `sports` s ON s.`id` = pc.`sport_id` WHERE pc.status = 1 AND pc.remove = 0 AND pc.team_id = ? AND p.`status` = 1 AND s.`status` = 1",{replacements:[team_id]});
            const [results, teams_challenges] = await sequelize.query("SELECT pc.id AS team_player_id,p.firstname,p.lastname,p.gender,p.age,p.token_id,p.`profile_img`,p.matches,p.won,p.draw,p.teams,p.teams_matches,p.teams_won,p.teams_draw,s.name AS sport_name,(SELECT GROUP_CONCAT(IF(pkc.won=p.id && pkc.draw=0,'W',IF(pkc.draw=1,'D','L'))) FROM players_challenges pkc WHERE (pkc.player_id = p.id OR pkc.opponent_id = p.id) and pkc.accept_status = 1 AND (pkc.won != 0 or pkc.draw != 0) ORDER BY pkc.id DESC LIMIT 5) as last_matches FROM `team_players` pc INNER JOIN `players` p ON pc.`player_id` = p.`id` INNER JOIN `sports` s ON s.`id` = pc.`sport_id` WHERE pc.status = 1 AND pc.remove = 0 AND pc.team_id = ? AND p.`status` = 1 AND s.`status` = 1 ORDER BY pc.id DESC LIMIT "+limit+" OFFSET "+offset+"",{replacements:[team_id]});
            if(teams_challenges!=""){
                var count_team = teams_challenges.length;
                var total_records_teams = challenges_total.length;
                res.status(200).json({status:true, total_records:total_records_teams, total:count_team, result:teams_challenges, message: count_team+" players team join request received!"});
