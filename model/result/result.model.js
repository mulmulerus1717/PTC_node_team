    ...
    try{
        ...
        if(searchteam !== "" && searchteam !== null && searchteam !== undefined){ queryBuilder += " AND (p.teamname LIKE '%"+searchteam+"%' OR pl.teamname LIKE '%"+searchteam+"%') ";}
            ...
            if(data == ""){//check already exist
                ...
                const [results, teamsResult] = await sequelize.query("SELECT pc.id AS challenges_id, pc.accept_status, IF(pc.draw=0,IF(pc.won!=0 && pc.won=pc.team_id,p.teamname, IF(pc.won!=0 && pc.won=pc.opponent_id, pl.teamname,'')),'') AS won, IF(pc.draw=0,IF(pc.won!=0 && pc.won=pc.team_id,p.profile_img, IF(pc.won!=0 && pc.won=pc.opponent_id, pl.profile_img,'')),'') AS winner_image, pc.draw, pc.result_date,IF(team_result!='draw',(SELECT teamname FROM teams WHERE id = team_result),team_result) as team_result, IF(opponent_result!='draw',(SELECT teamname FROM teams WHERE id = opponent_result),opponent_result) as opponent_result, s.name AS sports_name, p.profile_img as team_profile, p.teamname AS teamname, p.`token_id` AS team_token, pl.profile_img as opponent_profile, pl.teamname AS opponentname, pl.`token_id` AS opponent_token, pc.match_contest FROM teams_challenges pc INNER JOIN `teams` p ON p.id = pc.team_id INNER JOIN `teams` pl ON pl.id = pc.opponent_id INNER JOIN sports s ON s.id = pc.`sport_id` WHERE (pc.team_id = ? OR pc.opponent_id = ?) AND pc.block = 0 "+queryBuilder+" ORDER BY pc.id DESC LIMIT "+limit+" OFFSET "+offset,{replacements:[team_id,team_id]});
                if(teamsResult!=""){
    ...
    try{
        ...
        var auth_token = (authorization).split(" ")[1];
            ...
            if(data == ""){//check already exist
                ...
                const [results, teamsResult] = await sequelize.query("SELECT pc.id AS challenges_id, pc.accept_status, IF(pc.draw=0,IF(pc.won!=0,IF(p.id=pc.won,p.teamname,pl.teamname),0),0) AS won, pc.draw, pc.result_date,IF(team_result!='draw',(SELECT teamname FROM teams WHERE id = team_result),team_result) as team_result, IF(opponent_result!='draw',(SELECT teamname FROM teams WHERE id = opponent_result),opponent_result) as opponent_result, s.name AS sports_name, p.teamname AS teamname, p.`token_id` AS team_token, pl.teamname AS opponentname, pl.`token_id` AS opponent_token FROM teams_challenges pc INNER JOIN `teams` p ON p.id = pc.team_id INNER JOIN `teams` pl ON pl.id = pc.opponent_id INNER JOIN sports s ON s.id = pc.`sport_id` WHERE pc.id = ? AND (pc.team_id = ? OR pc.opponent_id = ?) AND pc.accept_status = 1 AND pc.block = 0",{replacements:[challenge_id,team_id,team_id]});
                if(teamsResult!=""){
